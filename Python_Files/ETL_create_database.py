...
def three_arguments_func(wiki_file, kaggle_file, ratings_file):
    ...
    # 9. Transform & merge the ratings DataFrame.
...
# Transform and merge the ratings DataFrame:
    ...
    # Creating database engine (PostgreSQL database)
    engine = create_engine(db_string)
    
    # Importing the movies_df DataFrame to a SQL database.
    movies_df.to_sql(name='movies', if_exists='replace',con=engine)
    
    # Creating a variable rows_imported
    rows_imported = 0
    
    # Setting start_time from time.time()
...
# 10. Creating the path to your file directory and variables for the three files.
...
three_arguments_func(wiki_file, kaggle_file, ratings_file)
